    ...
    public static partial class PWBIO
    {
        ...
        private static void ShapeToolEditMode(UnityEditor.SceneView sceneView)
        {
            ...
            else if (Event.current.type == EventType.KeyDown && Event.current.alt && Event.current.keyCode == KeyCode.Delete)
            {
                ...
                ShapeManager.instance.DeletePersistentItem(_selectedPersistentShapeData.id, !Event.current.shift);
            }
        }
        ...
        private static void ShapeStrokePreview(UnityEditor.SceneView sceneView, string hexId, bool forceUpdate)
        {
            ...
            for (int i = 0; i < brushstroke.Length; ++i)
            {
                ...
                if (strokeItem.settings.is2DAsset && sceneView.in2DMode)
                    itemRotation *= Quaternion.AngleAxis(90, Vector3.right);
