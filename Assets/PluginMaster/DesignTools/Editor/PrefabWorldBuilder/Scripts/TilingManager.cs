    ...
    public static partial class PWBIO
    {
        ...
        private static void TilingToolEditMode(UnityEditor.SceneView sceneView)
        {
            ...
            else if (Event.current.type == EventType.KeyDown && Event.current.alt && Event.current.keyCode == KeyCode.Delete)
            {
                ...
                TilingManager.instance.DeletePersistentItem(_selectedPersistentTilingData.id, !Event.current.shift);
            }
        ...
        private static void PreviewPersistentTiling(TilingData data)
        {
            ...
            for (int objIdx = 0; objIdx < objPos.Length; ++objIdx)
            {
                ...
                itemPosition -= pivotToCenter - itemRotation * (axisDirection * (size.y / 2));
