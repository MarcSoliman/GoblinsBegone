        ...
        private static void GravityToolDuringSceneGUI(UnityEditor.SceneView sceneView)
        {
                ...
                && (Event.current.type == EventType.MouseDown || Event.current.type == EventType.MouseDrag))
            {
                ...
                foreach (var item in _paintStroke)
                {
                    ...
                    if (UnityEditor.PrefabUtility.GetPrefabAssetType(item.prefab) == UnityEditor.PrefabAssetType.Model)
                        continue;
                    var meshFilters = item.prefab.GetComponentsInChildren<MeshFilter>();
                    ...
                    var skinnedMeshes = item.prefab.GetComponentsInChildren<SkinnedMeshRenderer>();
                    foreach (var skinnedMesh in skinnedMeshes)
                    {
                        var collider = skinnedMesh.gameObject.AddComponent<MeshCollider>();
                        collider.convex = true;
                        meshColliders.Add(skinnedMesh.gameObject.AddComponent<MeshCollider>());
                    }
                }
                ...
                foreach (var meshCol in meshColliders) Object.DestroyImmediate(meshCol, true);
                UnityEditor.AssetDatabase.Refresh();
            ...
            else if (Event.current.type == EventType.KeyDown && Event.current.control && Event.current.alt)
            {
                if (Event.current.keyCode == KeyCode.UpArrow || Event.current.keyCode == KeyCode.U)
                {
                    GravityToolManager.settings.height += Event.current.shift ? 0.1f : 1f;
                    ToolProperties.RepainWindow();
                }
                else if (Event.current.keyCode == KeyCode.DownArrow || Event.current.keyCode == KeyCode.J)
                {
                    GravityToolManager.settings.height -= Event.current.shift ? 0.1f : 1f;
                    ToolProperties.RepainWindow();
                }
