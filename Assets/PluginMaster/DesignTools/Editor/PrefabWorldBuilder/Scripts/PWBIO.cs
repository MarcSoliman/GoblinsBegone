    ...
    public static partial class PWBIO
    {
            ...
            Camera camera, bool redMaterial = false, bool reverseTriangles = false)
        {
            ...
            if (SpriteRenderers.Length > 0)
            {
                ...
                foreach (var spriteRenderer in SpriteRenderers) DrawSprite(spriteRenderer, rootToWorld, camera, bounds);
            }
        }

        private static void DrawSprite(SpriteRenderer renderer, Matrix4x4 matrix, Camera camera, Bounds objectBounds)
        {
            if (quadMesh == null)
            {
                quadMesh = new Mesh
            ...
            quadMesh.uv = new Vector2[] { new Vector2(minUV.x, maxUV.y),  new Vector2(maxUV.x, maxUV.y),
